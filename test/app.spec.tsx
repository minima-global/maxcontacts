import App from '../src/App'
import { commands, events } from 'npm-upload-9781'
import { waitFor, screen, fireEvent, render, act, getByText } from '@testing-library/react'
import { HashRouter, MemoryRouter } from 'react-router-dom'

const mockProfile = {
    logs: false,
    name: 'neil-desk',
    publickey:
        '',
    staticmls: false,
    mls: '@5.189.130.43:9001',
    localidentity:
        '@172.17.0.2:9001',
    contact:
        '@5.189.130.43:9001',
}

const mockContacts = {
    contacts: [
        {
            id: 196,
            publickey:
                '',
            currentaddress:
                '@167.86.99.207:9121',
            myaddress:
                '@64.227.37.67:9001',
            lastseen: 1668621955174,
            date: 'Wed Nov 16 18:05:55 GMT 2022',
            extradata: {
                name: 'Neil-phone',
                minimaaddress: 'MxG080MFYVD96N1TN4EKW6S8BRKMFADBFN9EHFE6NF28UTNEJ8MZ87ZFY6NWF5Y',
                topblock: '233642',
                checkblock: '233592',
                checkhash: '0x00000001765CE10E151C272C9165F1C6E52B682088361813DD813F4855A1DF95',
                mls: '@167.86.99.207:9121',
            },
            chaintip: '235205',
            samechain: true,
        },
        {
            id: 261,
            publickey:
                '',
            currentaddress:
                '@193.37.214.40:9001',
            myaddress:
                '@116.203.140.13:9001',
            lastseen: 1668699884252,
            date: 'Thu Nov 17 15:44:44 GMT 2022',
            extradata: {
                name: 'neil-desk',
                minimaaddress: 'MxG080SD8F1YJM8CJF77DVSH5JY7F2Z0ZBWZKZJJ0PEDECJ3FHCDF9VT5E1CE2S',
                topblock: '235181',
                checkblock: '235131',
                checkhash: '0x00000001D15D4EEE6C545F320C233CB05B401D3170CD2FD3565CB865D009BCD4',
                mls: '@5.189.130.43:9001',
            },
            chaintip: '235205',
            samechain: true,
        },
    ],
}

const mockedUsedNavigate = jest.fn()

beforeEach(() => {
    jest.spyOn(commands, 'maxima').mockResolvedValue(mockProfile)

    jest.spyOn(commands, 'maxcontacts').mockResolvedValue(mockContacts)

    jest.spyOn(events, 'onInit').mockImplementation((callback) => {
        console.log('mock init event fired immediately')
        callback()
    })
})

afterEach(() => {})

it('renders the App', async () => {
    const container = render(
        <HashRouter>
            <App />
        </HashRouter>
    )

    expect(screen.getByText('Contacts')).toBeTruthy()
})

it('renders the profile card', async () => {
    const container = render(
        <HashRouter>
            <App />
        </HashRouter>
    )

    expect(screen.getByText('Contacts'))

    await waitFor(() => screen.findAllByText('view your profile'))

    expect(screen.getByText('view your profile')).toBeTruthy()
})

it('clicking the profile card opens the profile page', async () => {
    const container = render(
        <HashRouter>
            <App />
        </HashRouter>
    )

    await waitFor(() => screen.findAllByText('view your profile'))

    const profileCard = screen.getByText('view your profile')

    fireEvent.click(profileCard)

    await waitFor(() => screen.getByText('Your Local Identity'))

    expect(screen.getByText(mockProfile.contact)).toBeTruthy()
})

// render from '@testing-library/react' not 'react-dom'
// dont use container (in beforeeach or tests) use screen instead
// expect(screen.getByText(mockProfile.contact)).toBeTruthy() not expect(screen.getByText(mockProfile.contact))
